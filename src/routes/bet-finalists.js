...
// VERSION SIN AUTENTICCIO 
router.get('/', controller.getAllFinalists) //obtiene todas los finalistas, accede cualquier usuario
router.get('/me/bet',  controller.getMeFinalists) //obtiene todas los finalistas, usuario logueado
router.get('/:id', controller.getFinalistById)  //obtiene cualquier finalista por id, cualquier user
router.post('/me/bet',  controller.addMeFinalist ) //crea finalista usuario logueado
router.post('/:iduser',  controller.addFinalist) //crea finalista a usuario con id ingresado por url, solo accede admmin
router.put('/me/bet/:id', controller.updateMeFinalist)//Actualiza finalista usuario logueado
router.put('/:id',  controller.updateFinalist) //Actualiza finalista id, solo accede admin
router.delete('/me/bet/:id', controller.deleteMeFinalist ) // Borra finalista id accede usuario logueado
router.delete('/me/bet', controller.deleteAllMeFinalists ) //borra todos los finalistas de usuario logueado
router.delete('/:id',  validar.isAdmin, controller.deleteFinalist)//borra finalista con id, accede admin
router.delete('/admin/:iduser', controller.deleteAllFinalistsByIdUser) // Borrar todos los finalistas de un usuario dado por id, accede admin
router.delete('/',  controller.deleteAllFinalists) //Borra las apuestas de todos los usuarios, solo accede admin
